...
class GlobalExceptionHandler {
    ...
    suspend fun handleIllegalArgumentException(exception: IllegalArgumentException): ResponseEntity<ErrorResponse> =
        ResponseEntity
            .status(HttpStatus.BAD_REQUEST)
    ...
    suspend fun handleNoResourceFoundException(): ResponseEntity<ErrorResponse> =
        ResponseEntity.status(HttpStatus.NOT_FOUND).body(ErrorResponse(HttpStatus.NOT_FOUND.value(), "찾을 수 없는 경로"))

    @ExceptionHandler(MethodNotAllowedException::class)
    suspend fun handleMethodNotAllowedException(): ResponseEntity<ErrorResponse> =
        ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED).body(ErrorResponse(HttpStatus.METHOD_NOT_ALLOWED.value(), "지원되지 않는 메서드 형식"))

    @ExceptionHandler(Exception::class)
    suspend fun handleRuntimeException(exception: Exception): ResponseEntity<ErrorResponse> =
        ResponseEntity
            .status(HttpStatus.INTERNAL_SERVER_ERROR)
            .body(ErrorResponse(500, exception.toString() ?: "Internal Server Error"))
}